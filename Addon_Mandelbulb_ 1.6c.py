...
# this is intermediate version of script with test OpenCL support through c++ precompiled DLL

# new version has also automated material creation in nodes tree with visualization of vertex colors. ver 1.0 needs manual node creation.
...
def add_object(self, context):
    ...
    if self.calc_type == 'PYTON':   # for python calculations
        ...
        if self.del_disconnected == True:
                    ...
                    if (voxels[i][1] == voxels[j][1]):
                        if (voxels[i][2] == voxels[j][2]):
                            if (voxels[i][0] < voxels[j][0] - interval*0.9 and voxels[i][0] > voxels[j][0] - interval*1.1 ) or (voxels[i][0] > voxels[j][0] + interval*0.9 and voxels[i][0] < voxels[j][0] + interval*1.1):
                                indices_connected[i][indice_b_count] = j
                                indice_c_count += 1
            # inside indices_connected[n][0-5] should be indices of connected vertices

            if self.fast_slow_del_disc :        # This is the fast algorithm. Its criterion is the number of surrounding voxels
                delete_list_indices = []
            ...
            else:                               # This is the slow algorithm. It groups voxels and checks how many are connected in each group
                current_vert_count = verts_count
                k = 0
                voxels_connected = []
                while True:
                    voxels_connected.append(k)
                    for o in range( 0, 6 ) :
                        if indices_connected[k][o] != -1 :
                            voxels_connected.append(indices_connected[k][o])
                    #m = k
                    connected_list_counter = 1
                    while ( connected_list_counter < len( voxels_connected ) ):
                        for l in range (0, 6):
                            if indices_connected[ voxels_connected[ connected_list_counter ] ] [l] != -1 :
                                for o in range( 0, len(voxels_connected)) :
                                    if voxels_connected[ o ] == indices_connected[ voxels_connected[ connected_list_counter ]] [l] :
                                        break
                                    if o == len(voxels_connected) - 1 :
                                        voxels_connected.append (indices_connected[ voxels_connected[ connected_list_counter ]] [l])

                        connected_list_counter += 1
                    if len( voxels_connected) <= self.del_disc_crit :

                        voxels_connected = sorted ( voxels_connected, reverse = True )
                        current_vert_count -= len( voxels_connected )
                        for f in range( 0, connected_list_counter):
                            to_del = voxels_connected[f]
                            print (to_del)
                            voxels.pop(to_del)
                            tmp_loop_col.pop(4*to_del+3)
                            tmp_loop_col.pop(4*to_del+2)
                            ...
                            t = 0
                            trem = 0
                            for t in range( 0, len( indices_connected )) :
                                for u in range (0, 6):
                                    if indices_connected[t][u] > to_del :
                                        indices_connected[t][u] -= 1
                                    if indices_connected[t][u] == to_del :
                                        trem = t
                            indices_connected.pop(trem)

                        verts_count -= len( voxels_connected )

                    voxels_connected.clear()
                    k += 1
                    if current_vert_count <=  k :
                        break

            '''
            are_connected = [False for x in range(verts_count)]
            already_iterated_through = [False for x in range(verts_count)]
            tmp_counter = 0
            for i in range(0, verts_count):
                are_connected [i] = True
                for c in range(0, 6):
                    if indices_connected [i][c] != -1:
                        are_connected [indices_connected [i][c]] = True
                already_iterated_through [i] = True

                while tmp_counter != verts_count:
                    for j in range( 0, verts_count ):
                        if are_connected [j] != -1:
                            for d in range(0, 6):
                                are_connected [indices_connected[are_connected [j]][d]] = True
                            already_iterated_through [are_connected [j]] = True
                            tmp_counter += 1
                            break
                connected_count = 0
                for v in range(0, verts_count):
                    if are_connected [v] == True:
                        connected_count += 1
                if connected_count < 5:      # connected max condition
                    for g in range(0, verts_count):
                        print(are_connected[g])
            '''
            '''
                        to_del = are_connected[g]
                        voxels.pop(to_del)
                        tmp_loop_col.pop(4*to_del+3)
                        tmp_loop_col.pop(4*to_del+2)
                        tmp_loop_col.pop(4*to_del+1)
        ...
        # This is the algorithm that calculates Delete internal. it is so slow because it has to iterate every voxel by every voxel  up to 3 times until hits are met.
        if self.del_internal == True:
            indices_internal = []       # this is a list for indices of internal voxels
            for i in range(0, verts_count):     # This loop iterates through every generated voxel and checks if it is internal voxel and deletes it
        ...
        os.chdir(r'directory with dll')
        compute_dll = ctypes.WinDLL( "mandelbulb_opencl_63n0m3.dll" )
        ...
        copy_data_api = copy_data_proto(("_Z14Export_voxel_dPf", compute_dll), copy_data_params)

        copy_vertex_face_data_proto = ctypes.WINFUNCTYPE (
            ctypes.c_int,
            ctypes.c_float,
            ctypes.POINTER(ctypes.c_float),
            ctypes.POINTER(ctypes.c_uint),
            ctypes.POINTER(ctypes.c_float) )
        copy_vertex_face_data_params = ( 1, "p1", 0 ), ( 1, "p2", 0 ),( 1, "p3", 0 ),( 1, "p4", 0 )
        copy_vertex_face_data_api = copy_vertex_face_data_proto(("_Z23Export_Faces_Vertices_dfPfPjS_", compute_dll), copy_vertex_face_data_params)
        ...
        if self.cub_or_vert == True:
            vertex_d = (ctypes.c_float*(8*3*7*verts_count))()
            face_d = (ctypes.c_uint*(4*6*7*verts_count))()
            colour_d = (ctypes.c_float*(4*7*verts_count))()
            copy_vertex_face_data_api( ctypes.c_float(half_vox_size), ctypes.cast(vertex_d, ctypes.POINTER(ctypes.c_float) ), ctypes.cast(face_d, ctypes.POINTER(ctypes.c_uint) ), ctypes.cast(colour_d, ctypes.POINTER(ctypes.c_float) ) )

            for i in range(0, verts_count):
                verts.append([ vertex_d[24*i], vertex_d[24*i+1], vertex_d[24*i+2] ])
                verts.append([ vertex_d[24*i+3], vertex_d[24*i+4], vertex_d[24*i+5] ])
                verts.append([ vertex_d[24*i+6], vertex_d[24*i+7], vertex_d[24*i+8] ])
                verts.append([ vertex_d[24*i+9], vertex_d[24*i+10], vertex_d[24*i+11] ])
                verts.append([ vertex_d[24*i+12], vertex_d[24*i+13], vertex_d[24*i+14] ])
                verts.append([ vertex_d[24*i+15], vertex_d[24*i+16], vertex_d[24*i+17] ])
                verts.append([ vertex_d[24*i+18], vertex_d[24*i+19], vertex_d[24*i+20] ])
                verts.append([ vertex_d[24*i+21], vertex_d[24*i+22], vertex_d[24*i+23] ])

                faces.append([ face_d[24*i], face_d[24*i+1], face_d[24*i+2], face_d[24*i+3] ])
                faces.append([ face_d[24*i+4], face_d[24*i+5], face_d[24*i+6], face_d[24*i+7] ])
                faces.append([ face_d[24*i+8], face_d[24*i+9], face_d[24*i+10], face_d[24*i+11] ])
                faces.append([ face_d[24*i+12], face_d[24*i+13], face_d[24*i+14], face_d[24*i+15] ])
                faces.append([ face_d[24*i+16], face_d[24*i+17], face_d[24*i+18], face_d[24*i+19] ])
                faces.append([ face_d[24*i+20], face_d[24*i+21], face_d[24*i+22], face_d[24*i+23] ])

                tmp_loop_col.append (colour_d[4*i])
                tmp_loop_col.append (colour_d[4*i+1])
                tmp_loop_col.append (colour_d[4*i+2])
                tmp_loop_col.append (colour_d[4*i+3])

        else:
            voxel_d = (ctypes.c_float*(7*verts_count))()
            copy_data_api( ctypes.cast(voxel_d, ctypes.POINTER(ctypes.c_float) ) )
...
class OBJECT_OT_add_object(Operator, AddObjectHelper):
    ...
    del_disconnected : BoolProperty(
        ...
        description = "delete disconnected voxels",
        default = False
    )
    fast_slow_del_disc : BoolProperty(
        name = "fast/accurate(slow) delete disconnected",
        description = "true = fast, false = accurate",
        default = True
    )
    del_disc_crit : IntProperty(
        name = "Delete disconnected size",
        description = "Criterion for deleting disconnected voxels. In accurate(slow) algorithm it is the max number of voxels in a connected bunch. In fast algorithm it is the max number of voxel's neighbouring voxels",
        default = 1
    )
    remesh : BoolProperty(
        name = "Apply remesh",
